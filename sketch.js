...
end4 = createSprite(600,650,250,50)




wall1 = createSprite(750,1450,450,5)
wall2  =createSprite(550,1250,5,450)
wall3  =createSprite(750,1100,5,400)
wall4 = createSprite(400,900,650,5)
wall5 = createSprite(350,1090,400,5)
wall6 = createSprite(930,1300,450,5)


key1 = createSprite(300,990,30,30)
key1.addImage(key)
key1.scale = 0.5
  

 jewel = createSprite(100,100,30,30)
...
function draw() {
  ...
  if(thief.isTouching(end1)){
    thief.x = 1100
    level+=1
  }

  if(thief.isTouching(end2)){
    thief.x = 1100
    level+=1
  }

  if(thief.isTouching(end3)){
    thief.x = 1100
    level+=1
  }
  ...
  if(gameState === PLAY){
    ...
    if(arrowGroup.isTouching(black1)){
      black1.destroy();
      score+=1
    }

    if(arrowGroup.isTouching(black2)){
      black2.destroy();
      score+=1
    }

    if(arrowGroup.isTouching(black3)){
      black3.destroy();
      score+=1
    }

    if(arrowGroup.isTouching(black4)){
      black4.destroy();
      score+=1
    }

    if(arrowGroup.isTouching(black5)){
      black5.destroy();
      score+=1
    }
